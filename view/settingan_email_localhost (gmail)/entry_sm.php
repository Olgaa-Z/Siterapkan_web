...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(isset($_GET['smid'])){
		...
		if($lihat_sm->rowCount() >= 1){
			...
			if(empty($fileName)){
				//echo "No Update File";
				$field = array('no_sm' => $nosm, 'tgl_terima' => $tgl_terimadb, 'tgl_surat' => $tglsmdb, 'pengirim' => $pengirim, 'klasifikasi' => $klasifikasi, 'tujuan_surat' => $tujuan, 'perihal' => $perihal, 'ket' => $ket);
			}else{
			...
			if(empty($fileName)){
				...
				if($insert->rowCount() >= 1){
					...
					if($EmailAccount->rowCount() >= 1 AND $AktifEmail->rowCount() >= 1){
						...
						$Rlayout = $isi;
						$arr = array("=NoAgenda=" =>sprintf("%04d", $noagenda), "=NoSurat=" => $nosm, "=Perihal=" => $perihal, "=TujuanSurat=" => $TujuanSurat, "=TglSurat=" =>tgl_indo($tglsmdb), "=TglTerima=" => tgl_indo($tgl_terimadb), "=AsalSurat=" =>$pengirim, "=Keterangan=" => $ket, "=Penerima=" =>$_SESSION['nama']);
