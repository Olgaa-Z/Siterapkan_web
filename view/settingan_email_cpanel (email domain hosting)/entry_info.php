			...
			if($EmailAccount->rowCount() >= 1 AND $AktifEmail->rowCount() >= 1){
				...
				if(is_array($_POST['tujuan'])){
					...
					if(!$mail->send()) {
						...
						echo "<script type=\"text/javascript\">alert('Data Berhasil diSimpan. Email notifikasi gagal dikirim!');window.location.href=\"./index.php?op=add_memo\";</script>";
					}else{
						echo "<script type=\"text/javascript\">alert('Data Berhasil diSimpan, Email notifikasi dikirim!');window.location.href=\"./index.php?op=add_memo\";</script>";
					}
				}
			}else{
