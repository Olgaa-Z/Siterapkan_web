			...
			<div class="col-xs-12"> <?php
				if($_SERVER["REQUEST_METHOD"] == "POST"){
					...
					if(isset($_GET['idklasik'])){
						...
						$Klasifikasi = $this->model->selectprepare("klasifikasi_sk", $field=null, $params, "id_klas=:id_klas");
						if($Klasifikasi->rowCount() >= 1){
							...
							$params = array(':id_klas' => $id_klas);
							$update = $this->model->updateprepare("klasifikasi_sk", $field, $params, "id_klas=:id_klas");
							if($update){
								echo "<script type=\"text/javascript\">alert('Data Berhasil diperbaharui...!!');window.location.href=\"./index.php?op=klasifikasi_sk\";</script>";
							}else{
						...
						$insert = $this->model->insertprepare("klasifikasi_sk", $field, $params);
						if($insert->rowCount() >= 1){
							echo "<script type=\"text/javascript\">alert('Data Berhasil Tersimpan...!!');window.location.href=\"./index.php?op=klasifikasi_sk\";</script>";
						}else{
					...
					if(isset($_GET['idklasik']) && empty($_GET['act'])){
						$id_klas = htmlspecialchars($purifier->purify(trim($_GET['idklasik'])), ENT_QUOTES);
						$params = array(':id_klas' => $id_klas);
						$cek_klas = $this->model->selectprepare("klasifikasi_sk", $field=null, $params, "id_klas=:id_klas");
						if($cek_klas->rowCount() >= 1){
							$data_cek_klas = $cek_klas->fetch(PDO::FETCH_OBJ);
							$title= "Edit Data Klasifikasi Surat Keluar";
							$kode = 'value="'.$data_cek_klas->kode .'" disabled';
							$nama = 'value="'.$data_cek_klas->nama .'"';
						}else{
							$title= "Entri Data Klasifikasi Surat Keluar";
						}
					}else{
						$title= "Entri Data Klasifikasi Surat Keluar";
					}
					if(isset($_GET['idklasik']) && (isset($_GET['act']) && $_GET['act'] == "del")){
						...
						if($lihat_sm->rowCount() >= 1){
							...
							$params = array(':id_klas' => $id_klas);
							$delete = $this->model->hapusprepare("klasifikasi_sk", $params, "id_klas=:id_klas");
							if($delete){
								echo "<script type=\"text/javascript\">alert('Data Berhasil di Hapus...!!');window.location.href=\"./index.php?op=klasifikasi_sk\";</script>";
							}else{
					...
					$GetKlasifikasi = $this->model->selectprepare("klasifikasi_sk", $field=null, $params=null, $where=null, "order by nama ASC");
											...
											<tr>
												...
												<td><center>
													<div class="hidden-sm hidden-xs btn-group">
														<a href="./index.php?op=klasifikasi_sk&idklasik=<?php echo $object->id_klas;?>">					
															<button class="btn btn-minier btn-info">
																<i class="ace-icon fa fa-pencil bigger-100"></i>
															</button>
														</a>
														<a href="./index.php?op=klasifikasi_sk&idklasik=<?php echo $object->id_klas;?>&act=del" onclick="return confirm('Anda yakin akan menghapus data ini??')">
